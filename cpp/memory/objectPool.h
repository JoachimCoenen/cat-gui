...
private:
	template<size_t _itemSize = _CtorDtor.getItemSize()>
	struct _Node
	{
	private:
		void *_memory;
		void *_lastDeleted;  // non-owning, might be null, only in this Node
		_Node *_nextNode;
		constexpr size_t _itemSize;
		size_t _capacity;
		size_t _emptySlots;
	public:
		_Node(size_t capacity) { // , size_t itemSize) {
			...
			_memory = TMemoryAllocator::Allocate(memorySize());
			if (_memory == nullptr) {
				std::clog << "  }} ERROR: std::bad_alloc()" << std::endl;
				throw std::bad_alloc();
			}

			_lastDeleted = nullptr;
			_nextNode = nullptr;
		...
		inline void* memory() { return _memory; }
		inline const void* memory() const { return _memory; }
		...
		constexpr size_t itemSize() const { return _itemSize; }
		inline size_t capacity() const { return _capacity; }
		inline size_t emptySlots() const { return _emptySlots; }
		inline size_t memorySize() const { return _itemSize * _capacity; }
		inline const void* lastSlot() const {
			char *baseAddr = (char *)memory();
			return (void *)(baseAddr + memorySize());
		}
		inline bool hasSlots() const { return emptySlots() > 0; }
		inline bool isEmpty() const { return emptySlots() == capacity(); }

	public:
		void freeObj(void *deletedObj) {
			// std::clog << "}} _Node.freeObj(" << (size_t)deletedObj << ")" << std::endl;
			if (isEmpty()) {
			...
			void** delObjPtrPtr = (void**)deletedObj;
			*delObjPtrPtr = _lastDeleted;
			_lastDeleted = deletedObj;
			_emptySlots++;
		}
		...
		void* getNextWithoutInitializing() {
			...
			if (! hasSlots()) {
				std::clog << "  }} ERROR: node already full. emptySlots=" << emptySlots() << ", capacity=" << capacity() << std::endl;
				throw std::length_error("node already full");
			}
			void* result;
			if (_lastDeleted) {
				// we have at least one deleted, that we can use:
				result = _lastDeleted;
				_lastDeleted = getPrevDeleted(_lastDeleted);
			} else {
				// there is no deleted item, so there are no holes and (_capacity - _emptySlots) is the first empty slot.
				size_t firstEmpty = capacity() - emptySlots();
				char *baseAddr = (char *)memory();
				result = (void *)((baseAddr) + firstEmpty * itemSize());
			}
		...
		void clearDeletedChain() {
			...
			void* deleted = _lastDeleted;
			while(deleted) {
				void* tmp = deleted;
				deleted = getPrevDeleted(deleted);
				*((void**)tmp) = nullptr;
			}
		}

	private:
		inline void* idxFromPtr(const void* deleted) { return *((void**)deleted); }

		inline void* getPrevDeleted(const void* deleted) { return *((void**)deleted); }
	};
...
private:
	std::pair<_Node*, _Node*> findNodeForPtr(void* pointer) {
		...
		_Node* aNode = _firstNode;
		while (aNode != nullptr) {
			if (aNode->memory() <= pointer && pointer <= aNode->lastSlot() ) {
